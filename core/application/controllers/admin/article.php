...
class article extends Admin_Controller {
   ...
   public function __construct()
	...
	public function index(){
		...
		if($this->model_article->validation() == TRUE){
			...
			$result = $this->model_article->save($data);
			$data['msg'] = 'success';
	      }
	...
	public function editArticle(){
		$data = $this->model_article->get($this->uri->segment(2));
		foreach($data as $row){
			$data['id']       = $row->id;
		...
		$data['allSection'] = $this->model_section->get();	 
	...
	public function UpdateArticle(){
		...
		if($this->model_article->validation() == TRUE){
						$data = array('date' =>adate(),
						  ...
						  'publish_up'=> $publish_up.date('His'),
						  'publish_down'=> $publish_down);
			$result = $this->model_article->save($data,$this->input->post('id'));
	...
	public function articleList(){
		...
		$data['article'] = $this->model_article->get_by(array('archive'=>'0'), 'DESC', 20, $this->uri->segment(2));
		$data['articleCout'] = $this->ablog->PageCount('articleList', $this->model_article->count_db() ,20, 2);
		$this->out('article','articleList',$data);
	}
	
	public function deleteArticle(){
		$this->model_article->delete($this->uri->segment(2));
		redirect('articleList');
	}
						
	public function archive(){
		$this->db->cache_delete('archive', 'index');
		$data['archive'] = $this->model_article->get_by(array('archive'=>'1'),'DESC');
		$this->out('article','archive',$data);
	}
		
	public function archive_article(){
		$this->model_article->save(array('archive'=>'1','archiveDate'=>adate(1)), $this->uri->segment(2));
		redirect('articleList');
	}
	
	public function restore(){
		$this->model_article->save(array('archive'=>'0','archiveDate'=>''), $this->uri->segment(2)); 
		redirect('archive');
	}
		
	public function showComment(){
		$this->model_article->save(array('comment'=>'1'),$this->uri->segment(2));
		redirect('articleList');
	}
		
	public function hideComment(){
		$this->model_article->save(array('comment'=>'0'),$this->uri->segment(2));
		redirect('articleList');
	}
	...
	public function section(){
		...
		$data['section'] = $this->model_section->get();
		$sectionCout = $this->model_section->count_db();
		$data['sectionCout'] = $this->ablog->PageCount($sectionCout);
		$this->out('article','section',$data);
 	}
	public function save_section(){
			
		$data = $this->model_section->array_from_post(array('title'));
		$this->model_section->save($data);
	    redirect('section');
		}
		
	public function delete_section(){
		$this->model_section->delete($this->uri->segment(2));
		redirect('section');
		}
	...
	public function edit_section(){
		...
		$query = $this->model_section->get_by(array('id'=> $this->uri->segment(2))); 
	...
	public function update_section(){
		...
		$data = $this->model_article->array_from_post(array('title'));
		$Update = $this->model_section->save($data,$id);
