...
class login extends CI_Controller {
   ...
   public function __construct()
	...
	public function validate_credentials(){
		...
		$result = $this->lib_database->can_log_in('a_admin_user',$query);
		$image = $this->lib_database->get_filde('a_admin_user',array('LoginName'=>$userName),'UserAvatar');
		$name = $this->lib_database->get_filde('a_admin_user',array('LoginName'=>$userName),'name');
		if($result == 1){
				$login = $this->lib_database->get_filde('a_web_config',array('id'=>1),'login');
				if($login == 1){
						$code = rand(111111,999999);
						$email = $this->lib_database->get_filde('a_admin_user',array('LoginName'=>$userName),'UserEmail');
						$mobile = $this->lib_database->get_filde('a_admin_user',array('LoginName'=>$userName),'UserMobile');
						$id = $this->lib_database->get_filde('a_admin_user',array('LoginName'=>$userName),'id');
	...
	public function passReset(){
			...
			$a = $this->password_reset->validate_user('a_admin_user',array('UserEmail'=>$email));
			echo json_encode($a);
		}
	...
	public function validate_code(){
		...
		if($input_code !=""){
			...
			$name = $this->lib_database->get_filde('a_admin_user',array('LoginName'=>$user_Name),'name');
