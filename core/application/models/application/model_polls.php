...
class model_polls extends MY_Model {  
		...
		public function insert_polls($question, $answer, $status){
			...
			$this->db->insert('a_polls',$polls);
			$this->db->order_by("id_polls", "desc");
			$this->db->limit(1);
			$result = $this->db->get('a_polls');
			...
			foreach($answer as $row){
				if($row !=''){
					...
					$this->db->insert('a_polls_answer',$data);
				}
				}
			}
		...
		public function insert_sub_polls(){
			...
			if( $title != '' ){
				...
				$result = $this->db->insert('a_polls_answer',$data);
				  return $result;
				}else{
					return FALSE;
					}
		...
		public function get_answer($id){
			...
			$query = $this->db->get('a_polls_answer');
			return $query->result();
			}
		...
		public function get_votes($id){
			...
			$query = $this->db->get('a_polls_votes');
			return $query->result();
			}
		...
		public function delete_polls(){
			...
			$result = $this->db->delete('a_polls');
			}
			
		public function delete_answer($id){
			$this->db->where('id_answer',$id);
			$result = $this->db->delete('a_polls_answer');
			return $result;
			}
					
		public function select(){
			$this->db->where('active','1');
			$data = $this->db->get('a_polls');
			return $data->result();
			}
		...
		public function save_answer($id_answer,$user_votes,$user_ip){
			...
			$this->db->insert('a_polls_votes',$data);
			}
